...
def list_harga_pecahan(profile) -> list[dict]:
    ...
    pembulatan margin yang meleset. Read-only; `harga_bulat` = pembulatan ke
    terdekat, penulisan tetap lewat update_harga per barang.
    """
    with mssql.cursor(profile) as cur:
        cur.execute(
    ...
    for r in rows:
        ...
        out.append({
            ...
            "harga_bulat": float(math.floor(harga + 0.5)),
        })
    return out

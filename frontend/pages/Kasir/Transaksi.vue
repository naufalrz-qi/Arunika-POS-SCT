...
const ppnbm = ref(0);
const pajak = ref(0);
const subtotal = (b) => ghb(Number(b.harga), [b.diskon1, b.diskon2, b.diskon3, b.diskon4]) * Number(b.qty || 0);
const total = computed(() => {
  const net = baris.value.reduce((s, b) => s + subtotal(b), 0);
  return net * (1 + (Number(pajak.value) || 0)) * (1 + (Number(ppnbm.value) || 0));
});
  ...
  <AdminLayout :title="label">
    ...
    <Deferred data="nota">
      ...
      <div class="grid gap-4 lg:grid-cols-3">
        ...
        <!-- Nota -->
        <Card class="lg:col-span-2" :title="label">
            ...
            <table class="w-full text-sm">
              ...
              <tbody>
                ...
                <tr v-for="(b, i) in baris" :key="i" class="border-b border-border-default">
                  ...
                  <td class="px-2 py-1 text-right">
                    <input v-model="b.harga" type="number" min="0" step="any" class="w-28 rounded-control border border-border-default bg-surface-1 px-2 py-1 text-right" />
                  </td>
                  <td class="px-2 py-1 text-right">

...
**Satu tujuan tidak boleh menjatuhkan sisanya — dan sempat terjadi (23–25 Sep 2026).** `sync_pair` memakai nama `mulai` untuk jam (`time.monotonic()`) lalu menimpanya dengan id feed, jadi `duration_ms` negatif, ditolak CHECK `core_synclog.duration_ms` SESUDAH commit, dan `IntegrityError`-nya menghentikan list comprehension `sync_all`. Tiap tick hanya toko pertama yang punya perubahan master yang dilayani; cursor PUSAT/RUMAK/TANJUNG macet di satu id selama dua hari, 117 traceback di log, nol baris di Kesehatan Sync. Tes lama tak menangkapnya karena memakai `from_id=0`, yang kebetulan membuat durasinya positif. Sekarang jam dan id dua variabel, dan `sync_all` menangkap exception apa pun per tujuan.
...
- **Pencocokan melihat detail juga** (`agregat_detail_harian`, JOIN ke header untuk tanggalnya; kolom numerik = irisan kedua sisi). Dulu hanya header, dan itu terbukti bolong 25 Sep 2026: **108 hari di 9 cabang** berbeda — semuanya `t_pembelian_detail` yang harga belinya dikoreksi sesudah lewat jendela 7 hari (mis. 6 Agu −82.500 di semua toko), plus 6 nota pembelian GUDANG yang barisnya diedit — dengan header identik dan hari beda **0**. Tabel detail lain cocok persis, jadi bukan artefak tipe kolom. Order detail yang berkunci `no_transaksi` (DRAGON/KOTA MAS/PAGESANGAN) dilewati, tidak menggagalkan cabangnya.
- **Slot harian hanya ditandai selesai kalau berhasil** (`scheduler._run_due_hub_pull`). Slotnya jam 02:00 (`HUB_MATCH_HOUR`) — saat semua cabang MATI. Dulu ditandai selesai apa pun hasilnya, jadi 23–25 Sep cocok, `hub_master`, dan master-ke-toko gagal timeout tiga malam berturut dan tak pernah diulang. Sekarang diulang tiap tick sampai server hidup. Master-ke-toko yang gagal ke SEBAGIAN toko tetap dianggap selesai (toko itu dijangkau `harga_sync`/`feed_sync`); gagal ke SEMUA toko diulang.

### Jam server legacy yang meleset

Semua jadwal legacy (SQL Agent `jobs_post`/`jobs_get`) dihitung dari jam server itu sendiri. **Jam yang mundur menghentikan job** sampai jam itu menyusul `next_run` lamanya. Terjadi di GUDANG sejak 23 Sep 2026: jam Windows-nya berulang kali mundur 8 jam (`GETDATE()` 07:40 +08:00 saat jam nyata 15:40; lompatan −448/−474 menit di `tbl_log_transaksi.waktu`), `jobs_post` berhenti di 12:55 dengan `next_run` 13:00, antrean `tbl_tmp_post` menumpuk ±300 baris per hari, dan **pembelian toko — yang tak lain penjualan GUDANG yang dikirim job itu lalu diunduh job GET toko (`kd_divisi` = kode pelanggan toko di GUDANG, mis. PUSAT = `CAA029`) — tertahan berjam-jam** tanpa satu pun galat. `tanggal` nota tetap benar (jam PC kasir); yang meleset `tanggal_server` dan `waktu` feed. Data yang tertunda akhirnya sampai begitu job jalan lagi; `EXEC msdb.dbo.sp_start_job N'jobs_post'` di GUDANG mengirim 313 baris dalam 2 menit. Jamnya sendiri harus dibetulkan di mesin GUDANG — tidak ada yang bisa dilakukan aplikasi ini selain memberi tahu: Kesehatan Sync kini membaca `GETDATE()` tiap server dan menandai selisih ≥ `JAM_MELESET_MENIT` (10) sebagai **mati** dengan penyebab "jam server meleset ±N jam". Jangan menulis pembelian ke toko sendiri sebagai jalan pintas: jalur legacy akan tetap mengirimnya kemudian, dan `t_pembelian_detail` tanpa PK akan menampung barisnya dua kali.
...
- Tampil di halaman **Pergerakan Harga** (`pergerakan_harga_index` → `PergerakanHarga.vue`), default perubahan HARI INI dengan toggle "Semua Riwayat", filter tanggal/kode/koneksi + info "snapshot terakhir". Tiap baris dibuka di modal edit yang sama dengan Update Barang (`BarangEditModal.vue` + endpoint `update-harga/detail`); Edit hanya aktif bila baris berasal dari koneksi aktif — endpoint tulis selalu menulis ke koneksi aktif server-side.
- **Saran Harga DIHAPUS (25 Sep 2026)**, di Update Harga, modal Edit Barang, dan Pergerakan Harga. Sumbernya tidak relevan: nominal di `keterangan` adalah catatan bebas ("ECER 3.450.000") yang di gudang/grosir justru harga orang lain, dan di grosir acuannya harga GUDANG yang sudah disamakan `harga_sync` tiap 60 detik — jadi daftarnya berisi usulan yang salah atau yang sudah terjadi. Audit **Harga Berpecahan** tetap (kolom `harga_bulat` = pembulatan ke rupiah terdekat), karena itu perbaikan data, bukan usulan harga.
...
## Nota Tanggal Mundur: penyebab dari jejak log (`reports.nota_mundur` + `riwayat_log.py`)

- **`tanggal_server` = waktu TERAKHIR disimpan, bukan waktu dibuat.** Edit nota di legacy menimpa `tanggal_server` DAN `kd_user` dengan waktu dan akun pengedit; `tanggal` tetap KECUALI edit memang mengganti tanggal (lihat butir berikut). Audit 2026-09-23 atas seluruh server produksi (penjualan Jan–Sep 2026): 465 dari 469 nota grosir/retail yang "mundur" ternyata diedit belakangan. GUDANG kebalikannya: 160 nota, nol edit — tanggal dipilih mundur saat input (nota komplain). Jadi `kd_user` t_penjualan bukan pembuat nota; pembuatnya ada di baris `t_penjualan__insert` di `tbl_log_transaksi`.
- **Edit BISA mengubah tanggal, dan nomornya ikut berganti.** Nomor dokumen = awalan(2) + YYMMDD + urut(4), dan YYMMDD selalu tanggal saat nomor itu dibuat (1,8 juta nota penjualan di 11 server: hanya 5 yang tak cocok, 4 bertahun mustahil). Edit yang mengganti tanggal menomori ulang dokumennya ke urutan tanggal baru — di log `key__no__LAMA; val__no__BARU;`. Terukur sepanjang log produksi: penjualan 61, pembelian 83, biaya operasional 222 (50 di 2026). **Klaim audit 2026-09-23 "nol edit yang mengubah tanggal" SALAH** — audit itu mengelompokkan versi menurut nomor BARU, padahal insert-nya tercatat dengan nomor LAMA. Akibatnya: pencarian update harus mencocokkan `;val__<no>__X;` (nomor sesudah edit), bukan awalan `key__`; riwayat harus menelusuri nomor lama (`riwayat_log.riwayat`); dan pembuat asli dicari dengan nomor lama. Sebagian "pemindahan" sebenarnya **pembetulan**: TANJUNG `ST2602080069` dibuat 15/09/2026 14.05 bertanggal 8 Feb (jam/tanggal salah), 46 menit kemudian dinomori ulang ke 15 Sep — insert-nya dekat waktu edit, bukan dekat tanggal nomor lama (jendela `a3`).
- **Nomor bisa dipakai ulang.** testGudang `GP2111270003`: dibuat Mei 2021 dengan tahun salah, dinomori bolak-balik September, akhirnya pindah ke `GP2011270008`; nomor yang kosong lalu dipakai pembelian LAIN di November. Arm B karena itu hanya menghitung pemindahan TERAKHIR ke sebuah nomor, dan hanya bila sesudahnya nomor itu tak dipindah pergi lagi. Diverifikasi terhadap pembacaan log Python independen: 156/156 (`Testing`) dan 64/64 (`testgudang`), 0 beda.
- **Tanggal tidak wajar** (< 2019-01-01 atau > besok) tak pernah muncul di laporan mana pun — semua laporan menyaring rentang tanggal, dan tahun 7252 tak pernah masuk rentang. Di produksi: ANDARIA 1 (7252), PAGESANGAN 3 (7202, 4105, 5202), GUDANG 1 retur pembelian bertanggal 2018 yang disimpan 2025. Data sungguhan paling awal 2019-11-20, dan tak ada dokumen wajar bertanggal masa depan — itu yang menetapkan `BATAS_TANGGAL_WAJAR`.
- **"Jam komputer salah" hanya untuk penjualan di server toko.** Ambang: ≥3 nota, satu akun, hari simpan sama, geser sama (dibulatkan ke jam), rentang simpan ≥60 menit. Di 10 server toko ia hanya menangkap satu kasus sepanjang sejarah — PRAYA 03/12/2025, 53 nota SUPERVISOR tercatat 02/12 sejak 08.22 sampai ±14.30 lalu kembali benar, yang memang jam komputer. Di GUDANG ambang yang sama menangkap 43 rombongan yang DISENGAJA ("DARI PUSAT LAMA…", komplain), dan di jenis lain (biaya operasional, koreksi stok) input susulan memang berombongan — karena itu `f["gudang"]` mematikannya dan hanya jenis Penjualan yang dinilai. Rombongan dihitung SEBELUM pencarian/divisi (lapis 2 vs 3), supaya mencari satu nomor tak mengubah labelnya.
- **Butuh index `IX_tbl_log_transaksi_aksi_waktu`** (`indexes.py`). Tanpanya satu-satunya index log adalah PK `id` (4,3 juta baris di PUSAT) dan setiap pencarian per nota adalah scan penuh. Layar mengecek keberadaannya (`views._log_siap`, hanya hasil positif yang di-cache) dan tanpa index menampilkan "Belum dicek" + pemberitahuan, bukan menggantung. Build terukur 5,3 dtk untuk 3,5 juta baris lokal — build offline menahan INSERT ke log, dan setiap simpan nota menulis log lewat trigger, jadi **bangun di luar jam toko**.
- **Pencocokan lewat jendela waktu, bukan scan**: baris log yang menyetel `tanggal_server` tercatat 0 dtk s.d. beberapa jam SESUDAH nilainya (≤2 dtk di ANDARIA/PAGESANGAN/GUDANG; s.d. 10 menit di PUSAT, yang mencap `tanggal_server` saat form dibuka). Diverifikasi baris-per-baris terhadap klasifikasi Python independen: 0 beda di 407 nota `Testing` dan 882 nota `testgudang`; 0,04 dtk untuk 3 tahun × 8 jenis dokumen.
...
- **Pilihan kertas melekat pada MESIN** (`localStorage`), bukan akun atau nota: satu PC kasir punya satu printer, dan yang di meja sebelah bisa lain. `?kertas=` menang atas simpanan supaya bisa dicoba sekali tanpa mengubah setelan mesin itu.
- **`Klaim Vou` berdiri sendiri, bukan dilebur ke Diskon.** `t_penjualan_total.total` SUDAH memotong nominal voucher — terukur di SERVER-TOYS, pada tiga nota bervoucher `bruto - total` persis sama dengan nominalnya. Tanpa dipisah, potongan itu tercetak sebagai "Diskon" dan pelanggan yang menyerahkan voucher Rp50.000 tak punya bukti vouchernya dipakai. Rumusnya jadi `diskon = bruto + pajak_rp - total - voucher`.
- **`Status` (LUNAS/KREDIT) dari `m_jenis_bayar.status`, BUKAN `t_penjualan.status`.** Yang terakhir praktis selalu 1 (523.878 baris berbanding 7) dan tak membedakan apa pun; yang pertama bernilai 1 untuk TUNAI/BON/DEBIT dan 2 untuk BG/CEK/KREDIT.
- **Label `Kasir` di struk diisi PEGAWAI yang melayani**, mengikuti template cetak yang dipakai toko. Nama pemilik akun yang menulis nota tetap ada di payload dan di laporan Penjualan per User.
...
- **Teks contoh installer legacy tak pernah jadi kop.** `g_info_profile` di SERVER-TOYS (18.927 baris) dan SERVER-GUDANG (15.698) masih berisi `PERUSAHAAN ANDA` / `ALAMAT PERUSAHAAN` / `telp = 0` apa adanya — nama toko yang sebenarnya justru ada di `m_divisi.nama`. `_terisi()` memperlakukan nilai-nilai itu sama dengan kosong, jadi urutan kop jadi: profil perusahaan yang benar-benar diisi → nama divisi notanya → menyerah. Perbaikan sesungguhnya tetap mengisi layar Informasi Perusahaan di server itu.
- **Tanda tangan hanya "Penerima".** Struk ini bukti terima barang; blok "Hormat kami" di sisi toko tak pernah dibubuhi siapa pun dan cuma menambah tinggi kertas.
...
- **Yang SENGAJA tak dicetak di struk pelanggan:** kode barang, kode pelanggan, dan nama kasir. Dua kode itu cuma berarti bagi orang dalam toko, dan yang memegang kertas ini pembelinya; membuangnya sekalian memberi nama barang lebar 40 kolom penuh. Nama kasir tetap ada di payload dan di laporan Penjualan per User untuk jejak audit — di kertas, yang berarti bagi pembeli adalah siapa yang MELAYANI (`Pegawai`), dan dua nama orang di satu struk cuma jadi derau.
- **Baris `Jumlah` menghitung BARIS, bukan qty.** Satuan tiap baris bisa berbeda (PCS, LUSIN, RTG), jadi menjumlahkan angka qty menghasilkan bilangan tanpa satuan yang justru menyesatkan. Yang dibutuhkan pembeli saat menerima barang adalah "ada berapa jenis" — qty per jenisnya sudah tercetak di barisnya.

...
## Koreksi stok di layar Opname (`apps/transactions/opname.py`)

Satu-satunya aksi TULIS yang duduk di halaman laporan. Bentuknya beda dari setiap jalur tulis lain, dan bedanya dipaksa database lama — semuanya diukur langsung di server (PAGESANGAN 4.917 baris, PUSAT, testgudang 6.699 baris):

- **`trig_update_stok_opname_stok` tidak aman multi-baris.** Isinya `SELECT @barang = inserted.kd_barang … FROM inserted` — assignment skalar dari sebuah tabel. Satu `INSERT` berisi banyak baris hanya menggeser stok untuk SATU baris; sisanya tetap tercatat, stoknya saja yang diam, tanpa galat. **Karena itu `buat_koreksi` menulis satu baris per `execute`, dan jangan pernah digabung "supaya efisien".** Terbukti di testgudang: 3 baris → stok ketiga barang naik 3. Trigger `DELETE` sama skalarnya, jadi pembatalan pun harus satu per satu.
- **Tabelnya datar**: satu `no_transaksi` = satu baris = satu barang. Tak ada tabel detail. Koreksi lima barang berarti lima nomor.
- **Arah koreksi ada di `status`, dan trigger yang mendefinisikannya**: `IF @status <> 2 SET @jumlah = @jumlah * -1`. Jadi `2` menambah stok, selain itu mengurangi. Data nyata memakai 2 (2.678) dan 3 (2.130); 0/1/4 sisa lama yang langka. Kita menulis 2 dan 3 — cocok dengan trigger, dengan data, dan dengan `rpt.opname`.
...
- **`kd_user` `char(6) NOT NULL`**, sedangkan akun admin/superadmin umumnya belum ditautkan ke user legacy. Layar mengatakannya di awal (prop `kd_user`) alih-alih menolak setelah sepuluh baris terisi. Tautannya diisi di Manajemen User — memalsukan kd_user berarti koreksi tercatat atas nama orang lain.

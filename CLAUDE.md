...
python manage.py cek_jejak       # verify the audit hash chain on ActivityLog; exit 1 at the first edited/deleted row
```
...
- `apps/master_data/master_crud.py` — one CRUD engine for **13 master tables** (pelanggan, supplier, and the 11 reference tables behind the single `kelola_referensi` menu). Adding a table means adding a `_MASTER` entry, not new SQL. Read `context.md` § "CRUD master & kas" before touching it — above all that master codes are `{huruf}{blok}{NNN}` with a **rolling block** (`MAA999` → `MAB000`; `m_merk` is already at `MAB483`), that the `MAX()` pattern must be `LIKE 'X[A-Z][A-Z][0-9][0-9][0-9]'` and never `LIKE 'X%'`, and that **there is no `DELETE` here and never will be** — `ON DELETE CASCADE` from `m_merk`/`m_kategori`/… reaches `m_barang`. Cancellation is `status = 0` — except in `m_biaya`, where `status` is **not an active flag at all**: it is the expense classification (1 = Operasional (Penjualan), 2 = Operasional (Adm. dan Umum), …), named `Jenis` by the legacy view `mon_m_biaya` and used as the filter by the two P&L views `mon_rl_biaya_penjualan` / `mon_rl_biaya_adm_dan_umum`. `m_biaya` has no active column at all. This entry used to read "active is 2 in m_biaya" — true-looking because all 38 testGUdang rows are 2, and wrong: grosirPusat has 6 rows at 1 and 26 at 2. The labels live in one place, `reports.JENIS_BIAYA`.
...
- `apps/transactions/laba_rugi.py` + `apps/inventory/services.nilai_persediaan` — **Laba Rugi**, replacing legacy `GetRekapHarian`. Read `context.md` § "Laba Rugi" before touching it. Two things decide everything here: the legacy function takes **171 s** (nested per-barang cursors), and its inventory valuation is **LIFO — prohibited by PSAK 14 / IAS 2** and prices opname-added stock at Rp 0. So this is deliberately **not** a faithful port: it values inventory at weighted average cost over cost-bearing inflows only (opening balance + net purchases − purchase returns), and **the screen carries a permanent banner saying the numbers will not match the old app**. Quantity is divisi-filtered, cost is not — that is what makes per-divisi statements sum exactly to the server total. A period entirely before the closing date raises `PeriodeTertutup` rather than being silently shifted forward the way legacy does. Never use `_purchase_prices()` for valuation — but not over units, which is what this entry used to say. It *did* divide both numerator and denominator by `bs.jumlah`, returning a price per *purchase* unit against a quantity in *base* units (10.00× on `AMP013`), and that inflated the `nominal` column on Stok Akhir. **Both are fixed**: the average now divides only the denominator, `harga_beli_akhir` likewise, and `check_stock_agg._cek_satuan_harga` holds the invariant (old formula: 181 of 26,118 barang violated on testgudang; current: zero on testgudang, PUSAT, PAGESANGAN). What remains true is the accounting reason: `_purchase_prices()` is not a PSAK 14 weighted average — no closing-date anchor, gross prices, purchase returns not subtracted. That one is `_harga_pokok_rata()`, and the two are deliberately ≤3% apart (Stok Akhir lists the whole catalogue, where the PSAK version values every barang without a purchase since closing at Rp 0 — 1,602 of 12,930 rows on PUSAT).
...
- `apps/transactions/edit_nota.py` — **Edit Nota Penjualan** (`/admin-panel/penjualan/edit-nota`, menu `edit_nota`: `tulis_kritis` + `butuh_tautan`, so admin by default and supervisor only as a superadmin grant). Read `context.md` § "Edit Nota Penjualan" first.
  - It **mimics the legacy app's edit exactly**, in this order: UPDATE the header first (editor's `kd_user`, `tanggal_server = GETDATE()`), then `DELETE TOP (1) FROM t_penjualan_detail` once per old row, then INSERT one row per execute, then the total. The header must go first because `riwayat_log` pairs item rows with the last header event before them.
  - That DELETE is the **one deliberate exception** to "never DELETE on a toko server". It is safe because the table is a leaf (no PK, so no FK and no cascade), it is scoped to one number, and the old content is already in the audit trail. The header is never deleted, which is why "batal nota" does not exist.
  - `tanggal`, `kd_divisi`, `status` and header `diskon1..4` are never written.
  - `tanggal_setor` is filled only when NULL: the legacy triggers concatenate it with `+`, so a NULL nulls the whole sync payload.
  - Refused before any write when: the nota is in a closed period or before the base stock snapshot, it has `t_piutang_cicilan`/`t_tagihan_detail`, its prefix is not its divisi's `kepala_nota` (a sync copy), or the screen's `versi` is stale. Refused attempts are logged too.
  - Verified against a mock legacy DB carrying the **real triggers** from `docs/skema/`.
- **Audit trail = `ActivityLog`, enriched rather than a second table** (`core/0019`). The added columns are `profile`/`profile_name`, `jenis_dokumen`/`no_dokumen`, `alasan`, and `data` (JSON: `skema`, `sebelum`, `sesudah`, `selisih`, `log_id`). Read `context.md` § "Jejak Audit".
  - Every new row is **hash-chained** in `ActivityLog.save()`, so even `objects.create` is covered. The row is hashed from a DB re-read, excluding the SET_NULL FKs, and the chain head is locked with `select_for_update` on `RantaiJejak`. `manage.py cek_jejak` finds tampering.
  - Log Aktivitas stays "my own trail" (`log_untuk`). **Jejak Audit** (`apps/monitoring/views_audit.py`, `teknis`) shows every account.
  - Per-nota history merges Arunika's rows with legacy `tbl_log_transaksi` events. Legacy events inside an Arunika edit's `log_id` range are badged "via Arunika".
  - Snapshots keep **legacy column names plus a `skema` marker**, and live in the pangkal DB, so the trail survives the migration to Arunika.
- `apps/transactions/riwayat_log.py` + `reports.nota_mundur` — **Nota Tanggal Mundur** reads `tbl_log_transaksi` to tell an edited nota from one input with an old date, and to name the original cashier. Read `context.md` § "Nota Tanggal Mundur" first: `tanggal_server` is the *last save*, not creation, and legacy edits overwrite `kd_user` with the editor — so `kd_user` on `t_penjualan` is not who made the nota. Log lookups need `IX_tbl_log_transaksi_aksi_waktu`; without it the screen degrades to "Belum dicek" rather than scanning millions of rows.
...
**`apps/bisnis` — Arunika's own schema, and the adapter that makes legacy speak it.** Design + rationale: `docs/superpowers/specs/2026-09-07-skema-arunika-rancangan.md` (§7 is the migration log; §8 the freeze checklist). Four files:

- `models.py` — **the single source of truth** for Arunika's own tables. Derived from business questions, never from legacy DDL; `hub_schema.baca_kolom()` is banned here (reading the vendor catalogue makes it a derivative work). No triggers, ever.
- `master_src.py` — one `_MASTER` dict per entity, each with a `"legacy"` and an `"arunika"` SELECT body, installed as `arunika_src.*` **views inside the Arunika database** (legacy receives zero objects). Plus `SUMBER_UTAMA` (which legacy table decides each entity's row count) and `TANPA_RTRIM`.
- `adapter.py` — bodies that must be **generated, not transcribed**: `pergerakan_stok` (iTVF from `inv._movement_sql`), `badan_penjualan`/`badan_pembelian`/`badan_penjualan_order`/`badan_retur_baris` (from `reports._nota_net()`/`_pembelian_nota()`/`_penjualan_order_net()`/`_line_net()`). A second copy of a money formula raises no error — it just returns different numbers.
- `test_adapter.py`, `test_master_src.py` — the invariants below are enforced here, not by review.
- **Moving data into a new Arunika DB** is a superadmin screen, *Transfer ke Arunika* (`/admin-panel/master/transfer-arunika`), driven by `transfer.py` on a background thread with progress in `apps.core.models.TransferArunika` (pangkal DB — never `apps/bisnis/models.py`, which is migrated into every Arunika DB). One run = two new `lingkungan=uji` profiles + three local DBs: `salin_legacy.salin` (raw legacy tables, date window, source only *read*) → `siapkan.siapkan_arunika` mode legacy → `siapkan_arunika` mode arunika → `muat.muat_semua`. The CLI commands `salin_legacy` / `init_arunika` / `isi_arunika` are thin wrappers over the same functions; errors meant for a human are `siapkan.Ditolak`. **Stock on the copied legacy profile is only correct if the window contains the source's last `g_tutup_buku` date** — the stock engine anchors there and walks forward/back (PUSAT Jan-2025 copy: 0 diffs on 31 Dec 2025, 9,741 on 31 Jan 2025). Arunika-shape reports are correct for any window. The run records `tutup_buku` and the screen warns (`stok_benar`).

**Reports read it through a twin.** A spec gains `"inner_arunika"` next to `"inner"`; `_pakai_bentuk_arunika()` picks it only when `ARUNIKA_LAPORAN` is on **and** the profile has an Arunika DB (default **off** — pulling latest code never moves a read path). Bespoke screens (Klasifikasi Pelanggan, Kas Harian) have no single `inner`, so they call `_arunika_siap(profile)` and must move **all** their routes at once — screen, export, and detail panel — or Excel and screen silently read different sources. `apps/monitoring/test_laporan_arunika.py` auto-discovers every twin via `dir(views)`; you do not register new ones.

Rules here, each bought by a failure:

- **Verify on both local profiles, never one.** `testgudang` (legacy `testGUdang`) and **`Testing`** (legacy `grosirPusat` — find profiles by `db_name`, not `name`). They are complementary, not big/small: `t_biaya_operasional` is 0 on gudang and 9.564 on grosirPusat; `t_mutasi_stok` the reverse. §8.1 has the table.
- **A legacy `status` column is usually not an active flag.** Three for three: `m_biaya` (expense class), `m_pegawai` (1 *and* 2 both active — `= 1` would kill 17 of 21 rows on grosirPusat), `m_jenis_bayar` (payment-instrument class). Before mapping one: count its distribution on **both** servers, then grep `sys.sql_modules` for a view that *filters* it. Nothing filters it → not a live/dead flag; map `aktif` constant and record the classification separately.
- **RTRIM `char`, never `varchar`.** `RTRIM(col) = ?` cannot seek an index (measured 2,72 s → 0,03 s). Every deliberate omission goes in `TANPA_RTRIM`, and `cek_arunika` re-checks the source is still `varchar` on the real server.
- **Header columns are GROUP BY keys, not `MIN()`.** An aggregate blocks the outer date predicate from descending below GROUP BY — the view is built with `"1=1"`, so *all* filtering is outer. Cost the same query 21× (measured twice: `_nota_net`, then `_order_net`). Check the key is unique first.
- **Row-by-row comparison needs a *total* order, and SQL `ORDER BY` cannot give it** — collation ignores trailing spaces, so `'X'` and `'X '` tie and swap freely. Sort in Python over the full tuple. This reported 19 then 8 differences that did not exist.
- Verification per step: `manage.py test`, then `init_arunika` + `cek_arunika` on **both** profiles, then row-by-row legacy vs Arunika over several ranges with timings in the commit message.

**MS SQL gotcha — pyodbc binds `str` as NVARCHAR.** Every legacy key column is `varchar`/`char`, so the implicit conversion lands on the *column* and kills the index seek. Two helpers, and picking the wrong one breaks things in a way local testing will not reveal:

- `core/mssql.execute_varchar(cur, sql, params)` — binds **only the ASCII `str` positions**, leaving everything else to pyodbc. Use this whenever the parameter list is mixed. Already wired into the two `_movement_sql` execute sites (`apps/inventory/services.py`) and all four in `apps/core/reporting.py`, which covers every server-side report and export. Measured: stock movement query (38 params, 22 of them `datetime`) **0.1368 s → 0.0064 s, 21.3×**; report `COUNT(*)` 2.7×, report page 1.5× — smaller there because the search filter is `LIKE '%…%'`, which was never seekable. Rows identical in every case; `check_stock_agg` still `OK: 35307 key identik`.
  Non-ASCII strings are deliberately left as NVARCHAR: a user-typed search term forced through VARCHAR is converted by the server codepage, which can map distinct characters onto the same byte and produce *wrong matches* with no error. Left alone it simply matches nothing, which is the correct answer against a `varchar` column.
- `apps/transactions/hub_sync.bind_varchar(cur, n, len)` — sets **every** position to VARCHAR. Correct only for a homogeneous `IN (...)` list, which is what it was written for.

Do **not** reach for `bind_varchar` on a mixed list. Forcing a `datetime` through VARCHAR hands date parsing to the server's language setting: measured on SQL Server 2022, it passes under `SET LANGUAGE us_english` and raises `DataError 22007` under `british` and `deutsch`. It looks fine on a dev machine and fails on the customer's server.
...
Two limits worth knowing before you assume a screen is covered. (1) A column named just `harga` is a selling price in Penjualan, a purchase price in Pembelian, and both at once in Transaksi Barang, so it sits under `nominal` only — an account that revokes *just* `harga_beli` still sees it. Fixing that needs column names that state their side, not another entry here. (2) The bespoke views (`kas_harian`, `fmi_stok`) are **not** in the name map, and this entry used to call that a gap. It is not one: they close their money columns through `_uang_bespoke()`, which checks the permission *key* instead of the column name, and `apps/monitoring/test_uang_e2e.UangBespoke` drives real HTTP requests to prove it. Their column names sit in `test_hidden_data.DI_LUAR_PETA_NAMA`, whose own comment says the same thing — registering them in the name map would blank Mutasi Stok, Opname Stok, and Transaksi Barang, where `masuk`/`keluar` are quantities.
...
All admin menus render real MS SQL data — `frontend/mock/*` has been deleted. Report pages come in two stacks that should not be mixed: the server-side stack (`ReportPage.vue` + `ServerTable.vue` + `useServerReport.js`; server pagination/sort/filter, XLSX export via the backend `/export` routes) used by `Reports/*`, `Promo/*`, `Cash/*`, `Analytics/*`, Opname, TransaksiBarang, and Master Produk; and the client-side stack (`ReportView.vue` + `DataTable.vue` + `useReportFilters.js`; in-browser sort/paginate, lazy-loaded SheetJS export) used by the inventory pages.
...
**Master Produk was moved onto the server-side stack** (2026-09-03). It used to ship the whole ~55k-row catalogue as list-of-dict and filter in the browser — the same disease that killed Stok Akhir — while its own filter panel was `@submit="() => {}"`, so `?search=` never reached the server and every load fetched everything. The symptom was a page that never finished loading. Its `stok` column was **dropped, not ported**: it read `m_barang_stok_akhir`, an abandoned legacy cache with no trigger on `t_pembelian_detail` or `t_mutasi_stok_detail`, so it only ever falls (22,592 of 22,703 rows negative, minimum −7,187,049). Real stock lives in Stok Akhir / Stok per Divisi, on the movement engine in `apps/inventory/services.py`.

...
- `apps/transactions/opname.py` — **koreksi stok**, the only write action living on a report page (Opname Stok). Read `context.md` § "Koreksi stok" before touching it — above all why it inserts **one row per `execute`**: `trig_update_stok_opname_stok` assigns scalars from `inserted`, so a multi-row INSERT moves stock for exactly one arbitrary row and the rest fail silently. Also why `kd_divisi` must be *chosen* (gudang has five divisi and all its opname lives in DAA001, not the first one) and why the number prefix comes from that divisi's `kepala_nota`.
...
**Routing.** `config/urls.py` mounts `apps.auth_app` at `/` and `apps.monitoring` + `apps.connections` under `/admin-panel/`. `apps/monitoring/views.py` holds most admin menu endpoints. RBAC roles: kasir / supervisor / admin / superadmin, with per-user `allowed_menu_keys`; menu definitions in `apps/core/menus.py`. Two menu flags override the per-user grants: `superadmin_only` (excluded from `assignable_menus()` entirely) and `admin_only` (still grantable — admins are managed through Kelola Menu — but dropped by `menus_for()` for kasir/supervisor no matter what is checked). Both are enforced in `menus_for()`, which `admin_network_guard` also reads, so a revoked menu cannot be reached by typing its URL — sub-paths included, via prefix matching.

...
- `apps/transactions/kas.py` — write path for `t_biaya_operasional`, `t_penambahan_kas`, `t_mutasi_kas`, modelled on `opname.py`. Note `kd_kas_tujuan` references `m_kas` despite its `varchar(10)`/`JR_KODE_ACCOUNT` type — three legacy views settle it; check `sys.sql_modules` before inferring column meaning from a schema.
